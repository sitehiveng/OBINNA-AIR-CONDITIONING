...
(function(){
  ...
  document.addEventListener("DOMContentLoaded", function(){
    ...
    /* Every message is tagged with the page it came from — this shows up
       directly inside the WhatsApp chat itself, so Obinna can see at a
       glance (no dashboard needed) that a real enquiry came from the site,
       and which page/product/service it came from. */
    var pageLabel = (document.title.split("|")[0] || "Website").trim();
    document.querySelectorAll("[data-wa]").forEach(function(el){
      var msg = el.getAttribute("data-wa-msg") || DEFAULT_MSG;
      var taggedMsg = msg + "\n\n(Sent via Obinna AC website — " + pageLabel + " page)";
      el.setAttribute("href", "https://wa.me/" + WHATSAPP_NUMBER + "?text=" + encodeURIComponent(taggedMsg));
    ...
    /* ---- Track social media link clicks (Facebook, TikTok) ---- */
    document.querySelectorAll(".footer-social a[href^='http']").forEach(function(el){
      el.addEventListener("click", function(){
        if(window.gtag){
          var platform = el.getAttribute("aria-label") || "social";
